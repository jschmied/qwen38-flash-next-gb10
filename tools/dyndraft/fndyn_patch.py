...
import os, sys
VENV = "/opt/llm/runtime/vllm-venv-main1ea7/lib/python3.12/site-packages/vllm"
if len(sys.argv) > 2:
    MTP, SPEC = f"{sys.argv[2]}/mtp.py", f"{sys.argv[2]}/speculator.py"
else:
...
S3N = """                self._generate_draft(
    ...
    def _fused_multi_step_decode(
"""
EDITS = {MTP: [(M1, M1N), (M2, M2N)], SPEC: [(S1, S1N), (S2, S2N), (S3, S3N)]}
off = sys.argv[1:2] == ["off"]
