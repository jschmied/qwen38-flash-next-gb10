...
sliced; the loop is an opaque custom op so torch.compile does not specialise on M. Env: VLLM_FP8_PAD_M4
...
NEW = '''    def apply_block_scaled_mm(
        ...
        out_dtype = self.config.out_dtype
        # ---- FP8 M4PAD (jschmied 2026-09-04): the sm_12x blockwise kernel routes M%4!=0 (and M<=64) to a
        # swap_ab path that is ~1.6x slower at prefill M (fixed upstream in #52775, C++). Pad M to a
        # multiple of 4 here instead; opaque custom op so compile does not specialise on M. ----
        if _FP8_PAD_M4 and A.shape[0] > 64 and A.shape[0] % 4 != 0:
