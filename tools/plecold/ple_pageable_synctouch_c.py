...
_SYNCTOUCH = os.environ.get("FN_PLE_SYNCTOUCH", "") == "1"   # FNSYNCTOUCH: the gather waits for this step's touch
...
def _pf_report() -> None:
    ...
    logger.warning(
        ...
        "majflt_during_touch p50=%s p90=%s | lookup_wait_ms p50=%s p90=%s",
        ...
        q("majflt", .5), q("majflt", .9), q("wait_ms", .5), q("wait_ms", .9))
...
class PagePrefetcher:
    ...
    def __init__(
        ...
        if _SYNCTOUCH:
            logger.warning("FNSYNCTOUCH armed: the PLE gather waits (<= 50 ms) for the step's CPU page touch; C helper %s", _POP_C is not None)
    ...
    def _serve(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                self.free.put(slot)
            f0 = _pf_majflt() if step is not None else 0
            try:
