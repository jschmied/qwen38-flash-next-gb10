...
def _pf_report() -> None:
    ...
    logger.warning(
        ...
        "majflt_during_touch p50=%s p90=%s",
        ...
        while True:
            ...
            if step is not None:
                try:
                    ...
                    evs = _PF_GATHER.pop(step, [])
                    lost = any(e0.query() for e0, _, _ in evs)      # GPU already started the gather
                    gus = None
                    if evs:
                        for _, e1, _ in evs:
                            e1.synchronize()
                        gus = sum(e0.elapsed_time(e1) for e0, e1, _ in evs) * 1000
                    _PF_STATS.append({"prefetch_ms": (t_done - t_prep) * 1000, "inputs_ms": (t_ready - t_prep) * 1000,
                                      "touch_ms": (t_done - t_ready) * 1000, "gather_us": gus, "lost": lost,
                                      "majflt": _pf_majflt() - f0})
