#!/usr/bin/env python3
"""Tile-union QSA sparse attention, v3 standalone (review 2026-09-04 items 1-3, 5):
...
def build(blocks, qpos, R, req):
    ...
    return uphys, umask, cnt, tails, T

@triton.jit
def _attn(q_ptr, k_ptr, v_ptr, uphys_ptr, umask_ptr, cnt_ptr, tail_ptr, out_ptr, stride_q_row, stride_q_head, stride_k_token, stride_k_head,
          stride_u, stride_tail, stride_out_row, stride_out_head, num_rows,
...
for R in (4, 2):
    uphys, umask, cnt, tails, T = build(blocks, qpos, R, REQ); torch.cuda.synchronize()
    t_pre=timeit(lambda: build(blocks, qpos, R, REQ))
    print(f"  R={R}: union mean {float(cnt.float().mean()):.0f} blocks/tile (x{float(cnt.float().mean())/max(1,float((blocks>=0).sum(1).float().mean())):.2f} of a row), precompute {t_pre:7.1f} us", flush=True)
    for BNB in (8, 16, 32):
        for warps in (4, 8):
            try:
                def run():
                    out=torch.empty_like(q)
                    _attn[(T, HKV)](q, kc, vc, uphys, umask, cnt, tails, out, q.stride(0), q.stride(1), kc.stride(1), kc.stride(2), uphys.stride(0), tails.stride(0), out.stride(0), out.stride(1), rows,
                                    R=R, GP=GP, G=G, D=D, BNB=BNB, CR=CR, TAIL_COLS=TAIL_COLS, num_warps=warps, num_stages=1)
                    return out
                out=run(); torch.cuda.synchronize(); diff=(out.float()-out_ref.float()).abs().max(); t=timeit(run)
                print(f"    R={R} BNB={BNB:2d} (BN={BNB*CR:3d}) warps={warps}: kernel {t:8.1f} + pre {t_pre:6.1f} = {t+t_pre:8.1f} us -> x{t_ref/(t+t_pre):4.2f} (kernel x{t_ref/t:4.2f})  max|diff|={float(diff):.4f}", flush=True)
            except Exception as ex: print(f"    R={R} BNB={BNB} warps={warps}: ERR {str(ex).splitlines()[0][:110]}", flush=True)
