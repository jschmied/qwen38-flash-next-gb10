...
for s,L in bystream.items():
    for i,r in enumerate(L):
        ...
        if sh:
            ...
            ov=0
            for q in sh:
                for g in gg: ov+=max(0,min(q[1],g[1])-max(q[0],g[0]))
            res['shared_overlap_with_GG_us'].append(ov/1000)
            res['shared_start_rel_router_us'].append((min(q[0] for q in sh)-rstart)/1000)
            big=[q for q in sh if S[q[2]]=='Kernel2']
            ov2=0
            for q in big:
                for g in gg: ov2+=max(0,min(q[1],g[1])-max(q[0],g[0]))
            res['shared_gemm_busy_us'].append(sum(q[1]-q[0] for q in big)/1000)
            res['shared_gemm_overlap_GG_us'].append(ov2/1000)
