POSTED 2026-09-05 13:0x as https://github.com/vllm-project/vllm/issues/55357#issuecomment-5551358118. Comment on vllm#55357 (tgmerritt, MTP episodic 0 % acceptance + degenerate output). Numbers: findings 126/127 (`notes/data/acceptcell2*`, `acceptcell3*`); bisect (finding 128) to be appended before posting.
...
Prompt length is not a factor: pairs of equal length, one token apart, 34 and 100 tokens apart all corrupt at the same rate (15/40). Not separated yet: MTP n=1 vs n=3 (multi-step draft decode), `--enforce-eager`, and `index_share_for_mtp_iteration`; running now, will add the result here. Finding with the per-cell data: https://github.com/jschmied/qwen38-flash-next-gb10/blob/b0856e2/notes/determinism-investigation.md#L1318 (raw texts per request in `notes/data/acceptcell2-*.jsonl`, `acceptcell3-*.jsonl`). Happy to run a specific configuration on this box.
