...
Code, test and all measurement logs: https://github.com/jschmied/qwen38-flash-next-gb10 — `tools/main/qsa_union_patch.py` (the kernels and the integration, currently as an env-gated patch on top of the nightly), `tools/qsa_union_test.py` (asserting test: tail lengths 0..CR−1, permuted physical pages, decoy requests, masked rows, both build paths), `notes/prefill-investigation.md` findings 92–115 (measurements, the bisect, and two server runs that were wrong and why).
