...
void persistent_topk_their(const torch::stable::Tensor& logits,
                         const torch::stable::Tensor& lengths,
                         torch::stable::Tensor& output,
                         torch::stable::Tensor& workspace, int64_t k,
                         int64_t max_seq_len);

STABLE_TORCH_LIBRARY(_C_their, ops) {
  ops.def(
      "persistent_topk(Tensor logits, Tensor lengths, Tensor! output, "
      "Tensor workspace, int k, int max_seq_len) -> ()");
}

STABLE_TORCH_LIBRARY_IMPL(_C_their, CUDA, ops) {
  ops.impl("persistent_topk", TORCH_BOX(&persistent_topk_their));
}
