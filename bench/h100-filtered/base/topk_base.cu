...
void launch_persistent_topk(const torch::stable::Tensor& logits,
  ...
  if (num_rows > 32 && max_smem_per_block >= 128 * 1024) {
    ...
    if (needs_cooperative && total_ctas > hw_resident_cap) {
      // BUNDLE EDIT (baseline arm only): upstream calls top_k_per_row_decode
      // here. That branch is unreachable on the >=128 KiB parts this bundle
      // targets, and pulling the decode kernel in would drag in ops.h and the
      // rest of the translation unit. Fail loudly instead of measuring a path
      // that is not the one under test.
      if (max_smem_per_block < 128 * 1024) {
        STD_TORCH_CHECK(false,
                        "baseline arm: low-smem decode fallback is not built "
                        "into this bundle; run it on a >=128 KiB device");
      }
      cudaError_t status =
          vllm::FilteredTopKRaggedTransform<float, int32_t, TopK>(
...
void persistent_topk_base(const torch::stable::Tensor& logits,
                     const torch::stable::Tensor& lengths,
                     torch::stable::Tensor& output,
