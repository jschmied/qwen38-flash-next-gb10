    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        if get_pp_group().is_last_rank:
            if config.tie_word_embeddings:
                ...
                # PATCH (jschmied 2026-08-28): same as model.py -- without
                # quant_config the MTP module installs UnquantizedEmbeddingMethod
                # and a quantized lm_head fails to load its scale companion.
                # Only reached when the head is NOT tied to embed_tokens.
                self.lm_head = ParallelLMHead(
                    config.vocab_size,
                    config.hidden_size,
                    quant_config=getattr(self, "quant_config", None)
                    or vllm_config.quant_config,
