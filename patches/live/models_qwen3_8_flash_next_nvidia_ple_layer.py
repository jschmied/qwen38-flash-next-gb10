...
def _get_ple_embedding_quant_method(
    ...
    if not isinstance(quant_config, Fp8Config):
        # PATCH (jschmied 2026-08-27): ModelOpt/NVFP4 checkpoints such as
        # RadixArk/Qwen3.8-Flash-Next-NVFP4 ship the PLE in exactly the format
        # this method implements -- F8_E4M3 shards plus ONE global BF16
        # `ngram_embedding.weight_scale` [1] -- but the isinstance gate rejects
        # them because the *body* is NVFP4, so the embedding is built
        # unquantized and loading dies with:
        #   ValueError: There is no module or parameter named
        #   'ngram_embedding.weight_scale' in Qwen3_8FlashNextNGramEmbedding
        _name = ""
        try:
        ...
        if _name in ("modelopt", "modelopt_fp4", "modelopt_mixed", "modelopt_mxfp8"):
            # Under PLE CPU offload the GPU process must NOT register
            # weight/weight_scale: load_weights() retains only
            # `_offload_weight_scale`, so a registered-but-unloaded
            # `weight_scale` shadows it in _get_embedding_weight_scale() and
            # the lookup is dequantized with an uninitialised value (fluent
            # garbage, no error). The offload worker owns the real weights.
...
class Qwen3_8FlashNextNGramEmbedding(PleOffloadLayer):
    ...
    def forward_impl(  # type: ignore[override]
        ...
        if output_buffer is not None:
            ...
            # PATCH: avoid index_select(out=<fp8 view>). `out=` into a
            # reshape() result is only correct if reshape returns a view, and
            # torch's fp8 coverage for the out= path is thin. Do the gather
            # normally and copy into the buffer instead.
            _sel = torch.index_select(
                self.ngram_embedding.weight, 0, ngram_ids.reshape(-1)
            )
            _ws = getattr(self.ngram_embedding, "weight_scale", None)
            if _ws is not None and _sel.dtype == torch.float8_e4m3fn:
                _sel = _sel.float() * float(_ws.flatten()[0])
                _sel = _sel.to(output.dtype)
            output.reshape(-1, self.head_dim).copy_(_sel)
            return output
    ...
    def get_offload_output_dtype(self, default_dtype: torch.dtype) -> torch.dtype:
        ...
        # PATCH: always hand back the model dtype. The worker dequantizes the
        # fp8 rows itself, so both the pinned CPU buffer and the GPU buffer are
        # BF16 -- structurally identical to a BF16-PLE checkpoint.
        ...
        if hasattr(self, "_offload_weight_scale"):
            # PATCH: hand the GPU a BF16 buffer and dequantize in the worker,
            # making an FP8-PLE checkpoint structurally identical to a BF16-PLE
            # one (which is known to work). Avoids the fp8 consumer path.
            return default_dtype
        return default_dtype
...
class Qwen3_8FlashNextPLELayer(nn.Module, MambaBase):
    ...
    def _dequantize_embeddings(
        ...
        self._dq_calls = getattr(self, "_dq_calls", 0) + 1
        if 6 <= self._dq_calls <= 9:
            import logging as _lg
            _e = embeddings.float()
            _ws = self._get_embedding_weight_scale()
            _lg.getLogger("vllm").warning(
                "[DQ] call=%d dtype=%s is_fp8=%s absmax_pre=%.6g nonzero=%d/%d "
                "scale=%s absmax_post=%.6g",
                self._dq_calls, embeddings.dtype, is_fp8(embeddings),
                _e.abs().max().item(), int((_e != 0).sum().item()), _e.numel(),
                None if _ws is None else float(_ws.flatten()[0]),
                (_e.abs().max().item() * (1.0 if _ws is None else float(_ws.flatten()[0]))))
