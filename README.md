...
| greedy determinism | reproducible sequentially — cold and warm, at every length — once **five** separate defects are fixed, and they are **jointly necessary**: with any one of them fixed alone, 280–334 of the ~333 disagreeing positions survive; with all of them, exactly 0 (det-184). One is merged upstream — the PLE state-stride fix, [vllm#55375](https://github.com/vllm-project/vllm/pull/55375) (peakcrosser7, merged 2026-09-05 14:02 as `28e605fb33`); we hit the same bug independently, opened #55467 and closed it as a duplicate, and put the GB10 evidence on theirs. It is present in the serving venv as **our hand-applied overlay**, not from the wheel: that nightly is `8340fe1bb9` (2026-09-04 20:27), eighteen hours older than the merge. The two implementations are semantically identical — the whole diff is the argument position of `state_idx_stride` and a hoisted local — so a venv bump replaces the overlay with upstream's equivalent. The other four are carried as overlays: deterministic `persistent_topk` ([vllm#55122](https://github.com/vllm-project/vllm/pull/55122), open), bit-stable MoE finalize, the FlashInfer autotune **cache key** (without it the finalize fix cannot even start — `Invalid gemm2 profile id`), and the **PLE offload semaphore reset** ([PR #13 on the #53899 branch](https://github.com/peakcrosser7/vllm/pull/13): with CUDA graphs on, every forward used the *previous* step's PLE outputs — the residual "noise" of four days, finding 138), which is not upstream in any form. Still not batch-invariant under concurrency |
